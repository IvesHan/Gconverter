        ...
        with st.container(border=True):
            ...
            with col_p2:
                correction_method = st.selectbox("矫正算法 (Correction):", 
                                                 ...
                                                 index=0, 
                                                 help="推荐使用 FDR (即 BH 方法)，这与 clusterProfiler 结果一致。")
            
            exclude_iea = st.checkbox("排除电子注释 (Exclude IEA)", value=False, help="❌ 取消勾选（即包含 IEA）能获得更多 GO 结果。")
    ...
    if run_enrich and raw_text_enrich:
        ...
        try:
            with st.spinner("正在进行富集分析 (g:Profiler)..."):
                gp = GProfiler(user_agent='streamlit_app_v3.3')
            ...
            if not isinstance(raw_results, dict) or 'result' not in raw_results or not raw_results['result']:
                # 安全地检查结果结构
                st.warning(f"⚠️ 分析完成，但在当前参数下未发现显著通路。")
                ...
                try:
                    ...
                    st.write(f"ID 诊断: 输入的前 10 个 ID 中，MyGene 成功识别了 {found} 个。请确认物种选择和 ID 格式。")
                except:
                    pass
